...
colnames(adultTest) <- c('age', 'workclass', 'fnlwgt', 'educatoin', 
                     'educatoin_num', 'marital_status', 'occupation', 'relationship', 'race', 'sex', 
                     'capital_gain', 'capital_loss', 'hours_per_week', 'native_country', 'income')
head(adultTest, 15)
...
#Copy Model into var
completeModel <- lreg
completeModel <- glm(income ~ 1, data = adultLog, family = binomial('logit'))
...
pred <- rep('below', length(probLog))
pred[probLog>=.5] <- 'above'
...
#Do the logistic regression
lreg <- glm(income~.,data=adultNine,family = binomial('logit'))
...
#Copy Model into var
completeModel <- lreg
completeModel <- glm(income ~ 1, data = adultNine, family = binomial('logit'))
...
rf3 <- randomForest(income ~ ., data = adult, ntree = 1000)
rf3.pred.prob <- predict(rf3, newdata = adultTest, type = 'prob')
rf3.pred <- predict(rf3, newdata = adultTest, type = 'class')
# confusion matrix 
tb3 <- table(rf3.pred, adultTest$income)
tb3
